...
@nki_jit
def nki_tensor_add_(residual, hidden, output): 
    '''
    Takes 2 input tensors of the same varying shape, and adds them.
    '''    
    assert residual.shape == hidden.shape

    # grab indexing tensors from the second and third dimensions
    P, F = residual[0].shape
    
    TILE_P = nl.tile_size.pmax # 128
    TILE_F = nl.tile_size.gemm_moving_fmax # 512

    # walk through the outer dimension, but only use NKI indexing on second and third
    for i in nl.affine_range(residual.shape[0]):

        for p in nl.affine_range(math.ceil(P / TILE_P)):
            
            i_p = p * TILE_P + nl.arange(TILE_P)[:, None]
    
            for f in nl.affine_range(math.ceil(F / TILE_F)):
    
                i_f = f * TILE_F + nl.arange(TILE_F)[None, :]
                
                residual_tile = nl.load(residual[i, i_p, i_f], mask=((i_p < P) & (i_f < F)))
                
                hidden_tile = nl.load(hidden[i, i_p, i_f], mask=((i_p < P) & (i_f < F)))
    
                res = nl.add(residual_tile, hidden_tile)
    
                nl.store(output[i, i_p, i_f], value = res, mask=((i_p < P) & (i_f < F)))
...
class NeuronLlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # try a kernel here
        hidden_states = nki_tensor_add_(residual, hidden_states, residual.clone())
