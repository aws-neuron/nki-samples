...
def fused_self_attn_for_SD_small_head_size(q_ref, k_ref, v_ref, use_causal_mask=False,
  ...
  for i_k_seq_tile in nl.affine_range(k_seq_n_tiles):
      ip_v_, if_v_ = nl.mgrid[0:v_seq_tile_size, 0:d_head_tile_size]
      trans_v[ip_v_, i_k_seq_tile, if_v_] = nl.load(
          v_ref[i_k_seq_tile * k_seq_tile_size + ip_v_, if_v_],
          dtype=pe_in_dt)

  q_local = nl.ndarray((q_seq_n_tiles, par_dim(d_head_tile_size), q_seq_tile_size), dtype=pe_in_dt)
  ip_q_, if_q_ = nl.mgrid[0:d_head_tile_size, 0:q_seq_tile_size]
  for i_q_seq_tile in nl.affine_range(q_seq_n_tiles):
      idx_1, idx_2 = nl.mgrid[0:q_seq_tile_size, 0:d_head_tile_size]
      q_local[i_q_seq_tile, ip_q_, if_q_] = nl.load_transpose2d(
          q_ref[i_q_seq_tile * q_seq_tile_size + idx_1, idx_2],
          dtype=pe_in_dt) * softmax_scale

  k_local = nl.ndarray((k_seq_n_tiles, par_dim(d_head_tile_size), k_seq_tile_size), dtype=pe_in_dt)
  ip_k_, if_k_ = nl.mgrid[0:d_head_tile_size, 0:k_seq_tile_size]
  for i_k_seq_tile in nl.affine_range(k_seq_n_tiles):
      idx_1, idx_2 = nl.mgrid[0:k_seq_tile_size, 0:d_head_tile_size]
      k_local[i_k_seq_tile, ip_k_, if_k_] = nl.load_transpose2d(
          k_ref[i_k_seq_tile * k_seq_tile_size + idx_1, idx_2],
          dtype=pe_in_dt)
